...
export class EmbeddingAnalysisService {
  ...
  private readonly pineconeNamespace =
    process.env.PINECONE_NAMESPACE ?? "code-pattern-errors";
  private readonly pineconeApiVersion =
    process.env.PINECONE_API_VERSION ?? "2026-04";
  private readonly similarityThreshold = Number(
    process.env.PINECONE_PATTERN_THRESHOLD ?? "0.78"
  );
  ...
  private detectLocalPatternIssues(code: string): EmbeddingAnalysisIssue[] {
    return LOCAL_PATTERN_ISSUES.filter((issue) => issue.pattern.test(code)).map(
      (issue) => ({
  ...
  private async searchPatternIndex(
    vector: number[]
  ): Promise<EmbeddingAnalysisIssue[]> {
    ...
    return (data.result?.hits ?? [])
      ...
      .map((hit) => {
        const severity =
          hit.fields?.severity === "error" ? "error" : "warning";
        return {
          severity,
          message:
            hit.fields?.message ??
